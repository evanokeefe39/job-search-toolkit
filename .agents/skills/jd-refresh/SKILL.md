...
new `jobs_ranked.csv`, then diff it against the previous run and present the
...
2. **Snapshot the prior ranked output** (before running anything).
   - If `jobs_ranked.csv` exists at the repo root, copy it to a gitignored path
     ...
     ```bash
     mkdir -p data && cp jobs_ranked.csv data/_tmp_jobs_ranked_prior_$(date +%Y%m%d_%H%M%S).csv
     ```
   - Note the exact snapshot path you created — you diff against it in step 4.
   - If `jobs_ranked.csv` does not exist, this is the first run: there is no
...
3. **Run the pipeline** (scrape → merge → enrich → score → export):
   ...
   - Outputs (repo root, both gitignored): `jobs_ranked.csv` (ranked list) and
     `merged_jobs.json` (canonical records).
...
4. **Compute and report the delta.** Diff the NEW `jobs_ranked.csv` against the
   snapshot from step 2, keyed on `apply_url` (the unique posting URL — the
   stable id across runs; it is also the `id` field in `merged_jobs.json`).
   Report:
...
- If the pipeline run fails (non-zero exit, Dagster materialization failure,
  ...
  exports).
- If the snapshot or the new `jobs_ranked.csv` is missing or unreadable at diff
...
- Never touch `applications/`, `resume/`, `tracker.csv`, or any file other than
  what the pipeline itself writes (`jobs_ranked.csv`, `merged_jobs.json`) and
  the gitignored snapshot from step 2.
- Never commit scraped outputs: `jobs_ranked.csv`, `merged_jobs.json`, and the
  scraper outputs are gitignored — leave them untracked.
