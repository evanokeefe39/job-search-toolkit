...
- Application folders: `applications/YYYY-MM-DD_<company>_<role>/` with `inputs/` (`jd.md`, `research.md`, `notes.md`) and `outputs/` (`cv_tailored.yaml`, `cv_tailored.pdf`, `rendercv_output/`); status is an append-only outcome feed via the `tracker` interface (`job-search-toolkit tracker record` — SQLite `data/tracker.db` by default, Twenty `tracker.backend: twenty` via config; never a hardcoded `../crm` call), synced into the warehouse by the `warehouse_outcomes` asset
