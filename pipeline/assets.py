...
# Paths
# ---------------------------------------------------------------------------
FREEWORK_RAW = REPO_ROOT / "freework_jobs.json"
HIRINGCAFE_RAW = REPO_ROOT / "hiringcafe_jobs.json"
MERGED_JOBS = REPO_ROOT / "merged_jobs.json"
RANKED_CSV = REPO_ROOT / "jobs_ranked.csv"
...
def freework_jobs() -> dg.MaterializeResult:
    ...
    subprocess.run(
        [sys.executable, str(REPO_ROOT / "scrape_freework.py"),
         "--format", "json", "--output", str(FREEWORK_RAW)],
        cwd=str(REPO_ROOT),
        check=True,
    )
...
def hiringcafe_jobs() -> dg.MaterializeResult:
    ...
    subprocess.run(
        [sys.executable, str(REPO_ROOT / "scrape_hiringcafe.py"),
         "--output", str(REPO_ROOT / "hiringcafe_jobs")],
        cwd=str(REPO_ROOT),
        check=True,
    )
